...
def is_outline(table: np.array, col: int, row: int):
...
def draw_outline(data: np.array):
    for i in range(1, 30):
        for j in range(1, 30):
...
def main(args):
    for filename in glob.glob(os.path.join(args.folder, '*.png')):
        with Image.open(os.path.join(os.getcwd(), filename)).convert('RGBA') as img:
            data = np.array(img)
            draw_outline(data)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Add black outline around a sprite")
    parser.add_argument(
        "folder",
